...
// Sorting By Height
let experiencedPlayersSortedByHeight = experiencedPlayers.sort {
        return $0["Height"] < $1["Height"]
}

let inexperiencedPlayersSortedByHeight = inexperiencedPlayers.sort {
    return $0["Height"] > $1["Height"]
}
...
dividePlayersIntoTeams(experiencedPlayersSortedByHeight)
dividePlayersIntoTeams(inexperiencedPlayersSortedByHeight)

func sendLetterToPlayers(teamOfPlayers: [[String : String]], teamName: String, location: String) {
    for player in teamOfPlayers {
        let name = player["Name"]!
        let guardianName = player["Guardian Name(s)"]!
        
        print("Dear \(guardianName),\n We would like to welcome your son/daughter, \(name), to the \(teamName).\n Our first practice will be on Wednesday, July 6th at 7:30 PM at \(location).\n We are looking forward to meeting you and to a fantastic season!")
    }
}

sendLetterToPlayers(dragons, teamName: "Dragons", location: "Cale Elementary School")
sendLetterToPlayers(sharks, teamName: "Sharks", location: "Murray High School")
sendLetterToPlayers(raptors, teamName: "Raptors", location: "the back fields at Darden Towe Park")

func calculateAverageHeightOfTeam(teamOfPlayers: [[String : String]]) -> Double {
    ...
    let averageHeight = totalTeamHeight/Double(teamOfPlayers.count)
    return averageHeight
}

calculateAverageHeightOfTeam(sharks)
calculateAverageHeightOfTeam(dragons)
calculateAverageHeightOfTeam(raptors)
